...
module Bbrcobranca
  module Boleto
    class ItauAdimplere < Base # Banco Itaú
      ...
      def codigo_barras
        instance_variable_get("@codigo_barras")
      end

      def logotipo
        if Bbrcobranca.configuration.gerador == :rghost_carne
          File.join(
            brcobranca_path, "lib", "bbrcobranca", "arquivos", "logos", "itau_carne.eps"
          )
        else
          File.join(brcobranca_path, "lib", "bbrcobranca", "arquivos", "logos", "itau.eps")
        end
      end
